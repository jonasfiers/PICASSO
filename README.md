...
- **Nested `OCCURS … DEPENDING ON`** (a *variable-length* table of tables) — an ODO table nested inside another `OCCURS`, or any `OCCURS` (fixed or ODO) nested inside an ODO table, is rejected with its own named error: the variable table's offset would itself be index-dependent, a shape the read-once-per-table count model does not express. Nested **fixed-count** `OCCURS` (a table of tables where every level has a fixed count) *is* supported and flattens recursively — see [What it does](#what-it-does).
