...
- **Decodes a single variable-length `OCCURS … DEPENDING ON` table** (ODO) — the common case: one `OCCURS m TO n … DEPENDING ON dep` per record, where `dep` is a field defined *before* the table. Because the count is data — read from `dep` per record at decode time — an ODO record has no single fixed layout the way everything above does; PICASSO models it separately. The parse still produces a tree, but marked variable-length (`ParsedCopybook.IsVariableLength`, with an `OdoInfo` descriptor), and the concrete byte layout is built *per record*: the codec reads `dep`, validates `m ≤ count ≤ n` (out-of-range fails loudly), expands the table to that many 1-indexed copies (`TAB(1)…TAB(count)`, trailing fields shifted past them), and decodes/encodes against it — round-tripping byte-for-byte, verified against GnuCOBOL's own record lengths at several counts. Everything harder stays rejected, each distinctly: two ODO tables in one record, an ODO nested in another `OCCURS`, an `OCCURS` nested inside the ODO, a `dep` that comes after the table or doesn't exist. This lives in the engine (`FlatFileCodec`'s `ParsedCopybook` overloads); the Integration Studio JSON action surface — whose flat spec is static — does not expose it yet and rejects an ODO copybook loudly rather than emit a spec that would silently mis-decode.
...
- **`OCCURS … DEPENDING ON` beyond the single-table case** — the common single-ODO case *is* supported at the engine level ([What it does](#what-it-does)). What stays rejected, each with its own named error: **more than one** ODO table in a record (its shape would depend on two counts at once), an ODO table **nested inside another `OCCURS`**, an `OCCURS` (fixed or ODO) **nested inside** the ODO table, and a **`DEPENDING` field defined after the table or absent** (the count must be readable before the variable section begins). The Integration Studio **JSON action surface** also does not expose ODO — its flat spec is static — so `ParseCopybook` rejects any ODO copybook loudly rather than emit a spec that would silently mis-decode; wiring the variable layout through that boundary is a separate, deliberate follow-up.
...
- **Float, DBCS and pointer `USAGE` clauses** — `COMP-1`/`COMP-2` (single/double float), `COMP-6`/`COMP-X` (Micro Focus dialect binary), `SYNC`/`SYNCHRONIZED` (alignment padding), `INDEX`, `POINTER` (and the `POINTER-32`/`POINTER-64`/procedure/function-pointer forms), `OBJECT REFERENCE`, `NATIONAL` and `DISPLAY-1` (double-byte), and `UTF-8` are each rejected with a named error. The supported `DISPLAY` sizing is one byte per digit or character; every one of these has a different physical width — a 4- or 8-byte float, a double-byte character, an aligned offset, an implementation-defined pointer — that this parser doesn't compute, so silently skipping the clause would mis-size the field (and `COMP-1`/`COMP-2`, which carry no `PIC`, would drop the field entirely and shift every offset after it). `COMP-1`/`COMP-2` float stays unsupported pending a deliberate binary-floating-point format decision; the Micro Focus `COMP-6`/`COMP-X` are rare enough to defer. The binary integer usages (`COMP`/`COMPUTATIONAL`, `COMP-4`, `COMP-5`, `BINARY`), `PACKED-DECIMAL` (an exact synonym for `COMP-3`), and `COMP-3` (`COMPUTATIONAL-3`) packed decimal *are* supported — see [What it does](#what-it-does).
