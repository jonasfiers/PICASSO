...
- **Binary, float and packed `USAGE` clauses** — `COMP`/`COMPUTATIONAL`, `COMP-1`/`COMP-2` (single/double float), `COMP-4`/`COMP-5`/`COMP-6`/`COMP-X`, `BINARY`, `PACKED-DECIMAL`, `SYNC`/`SYNCHRONIZED` (alignment padding), `INDEX`, `POINTER` (and the procedure/function-pointer forms), and `NATIONAL` (double-byte) are each rejected with a named error. The supported `DISPLAY` sizing is one byte per digit or character; every one of these has a different physical width — a binary halfword/fullword, a 4- or 8-byte float, packed nibbles, an aligned offset — that this parser doesn't compute, so silently skipping the clause would mis-size the field (and `COMP-1`/`COMP-2`, which carry no `PIC`, would drop the field entirely and shift every offset after it). `PACKED-DECIMAL` is a synonym for `COMP-3` but is rejected here rather than aliased to it — decoding it is a separate, deliberate step. `COMP-3` (`COMPUTATIONAL-3`) packed decimal *is* supported — see [What it does](#what-it-does).
