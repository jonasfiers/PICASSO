...
- **Decodes the named binary usages** — `BINARY-CHAR`, `BINARY-SHORT`, `BINARY-LONG`, `BINARY-DOUBLE` (and their `SIGNED` / `UNSIGNED` qualifiers). Unlike `COMP`/`BINARY`, these carry no `PIC`: the byte width is **fixed by the usage name** — 1, 2, 4, 8 bytes respectively — stored big-endian, two's-complement when `SIGNED` (the default) or plain magnitude when `UNSIGNED`. Signedness changes the value, not the width: a 1-byte `0xFF` decodes to `-1` signed but `255` unsigned; a 4-byte `0xFFFFFFFF` to `-1` vs `4294967295`. This closed a **silent miscompute** — a pic-less named-binary item used to be dropped entirely (no `PIC` → no field), sliding every following field forward and under-sizing the record; now it is sized from the usage and fails loud on anything ambiguous. A `PIC` clause combined with a named binary usage (its fixed width would contradict the digit count), and the platform-dependent `BINARY-C-LONG` (no standard width to assume), are both **rejected loudly** rather than guessed. This is what makes the DB2 `SQLCA` copybook — `COPY`'d into essentially every DB2 program, and full of `BINARY-LONG`/`BINARY-SHORT` — parse correctly (157 bytes, matching GnuCOBOL).
...
- **Reads both source formats** — free-format (`*>` comments) and traditional fixed-format, where columns 1–6 are a line-sequence number, column 7 flags a comment (`*`) or a continuation (`-`), and columns 73–80 are an identification area to ignore — dropped even when columns 1–6 are left blank, provided a space gap separates the code from an all-numeric trailing sequence number (so a free-format literal running past column 72 is never truncated). A `-` in column 7 continues the preceding line: an alphanumeric literal split across two lines (left open on the first, reopened with a quote in the continuation's area B — the NIST `K1WKA`/`K1WKY` shape) is rejoined into one properly-closed literal before tokenizing, so it is no longer mis-read as unterminated; a continuation that can't be reconstructed still fails loud. Detection is per line, so the two formats can mix in one file; no mode flag to pass.
...
- **Expands fixed-count `OCCURS`, including nested (a table of tables)** — a repeating item (`OCCURS n TIMES`, `TIMES` optional) is flattened into `n` 1-indexed fields with sequential offsets, so the flat layout stays flat and every downstream consumer works unchanged. An elementary field becomes `SLOT(1)`…`SLOT(n)`; a group carries the index into every descendant, so `LINE-ITEM OCCURS 3` over `ITEM-CODE`/`ITEM-QTY` gives `LINE-ITEM(1)-ITEM-CODE`, `LINE-ITEM(1)-ITEM-QTY`, `LINE-ITEM(2)-ITEM-CODE`, and so on. An `OCCURS` item **nested inside another** `OCCURS` (fixed count on both, to arbitrary depth) flattens recursively: the outer table expands to `N` copies, each holding `M` copies of the inner, and every level's index rides in the name — `LINE OCCURS 3` over `ITEM OCCURS 4` over `CODE` gives `LINE(1)-ITEM(1)-CODE`@0 … `LINE(3)-ITEM(4)-CODE`@22, one `(index)` segment per `OCCURS` level, the outer element's stride being the full byte size of one fully-expanded inner element. The parenthesized index is the whole disambiguation: `(` and `)` are illegal in a COBOL identifier, so an expanded name can never collide with a real one. `INDEXED BY` / `ASCENDING KEY IS` sub-clauses are tolerated and ignored. Only *fixed*-count nesting is supported; any nesting that involves an `OCCURS … DEPENDING ON` table stays rejected — see below.
...
- **Against a real COBOL compiler.** [`tools/layout-oracle`](tools/layout-oracle) is a differential harness: it compiles each copybook with **GnuCOBOL** (`cobc`, configured for IBM/MVS sizing), reads the authoritative `LENGTH OF` the whole record straight from the compiler, and diffs that against PICASSO's computed record length. Run across the broad external corpus PICASSO is exercised on — several hundred real copybooks from [AbsaOSS/cobrix](https://github.com/AbsaOSS/cobrix), [JRecord](https://github.com/bmTas/JRecord), and [AWS CardDemo](https://github.com/aws-samples/aws-mainframe-modernization-carddemo) — PICASSO's record length matches the compiler on every copybook it parses that `cobc` can also build, with a **single systematic exception**: the deliberate COMP-5 sizing dialect documented above (PICASSO uses IBM 2/4/8-byte sizing; native GnuCOBOL is byte-granular). Every new layout feature — REDEFINES, SYNC, nested/group `USAGE`, `OCCURS … DEPENDING ON` — is gated on an oracle pass before it merges.
...
- **_Nested_ `OCCURS … DEPENDING ON`** (a *variable-length* table of tables) — the supported ODO shapes are all **flat and top-level**: a single ODO table, or *several* ODO tables side by side in one record (counts resolved left-to-right, `OCCURS 0 TO n` included), are fully decoded and encoded ([What it does](#what-it-does)). What stays rejected, with its own named error, is **nesting a variable-length table** — an ODO table **inside another `OCCURS`**, or **any `OCCURS` (fixed or ODO) inside an ODO table's subtree**: the inner table's offset would itself be index-dependent, a shape the read-each-count-once model does not express. (Nested **fixed-count** `OCCURS` — every level a fixed count — *is* supported and flattens recursively; only variable-length nesting is out.) Also rejected, distinctly: a **`DEPENDING` field defined after its table or absent** (the count must be readable before the table's section begins). Separately, the Integration Studio **JSON action surface** does not expose ODO at all — its flat spec is static — so `ParseCopybook` rejects any ODO copybook loudly rather than emit a spec that would silently mis-decode; wiring the variable layout through that boundary is a deliberate follow-up.
...
The full [commit history](https://github.com/jonasfiers/PICASSO/commits/main) is public — commits are authored by Jonas, with Claude credited in the `Co-Authored-By` trailer — and this section states the split plainly rather than leaving it to be inferred from commit metadata.

## License

MIT — see [LICENSE](LICENSE) — with one exception: the DTAR020 sample files (`src/Picasso.Core/Samples/dtar020/DTAR020.cbl` and `DTAR020.bin`) are redistributed from [bmTas/CobolToJson](https://github.com/bmTas/CobolToJson) under **LGPL-2.1** and remain under that license, not MIT. See the [sample NOTICE](src/Picasso.Core/Samples/dtar020/NOTICE.md).
