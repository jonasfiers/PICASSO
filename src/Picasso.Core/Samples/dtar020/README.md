...
3. **`DTAR020.bin` has no record delimiters — now handled.** It's 10,233 bytes: 379 × 27, a bare concatenation with nothing between records. `FlatFileCodec` split records on newlines — a Unix convention, not a mainframe one — so it handed the layout the whole 10,233 bytes as one oversized "record". `RecordFormat.FixedLength` slices it by the layout's own width instead.
