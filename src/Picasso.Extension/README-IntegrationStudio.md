...
## Step 3 — Define the seven actions
...
All parameters are **Text** unless noted — two are **Binary Data**, called out where they occur.
...
### `DecodeRecordsFromBinary`

| Parameter | Direction | Type |
|---|---|---|
| `FlatSpecJson` | Input | Text |
| `FixedWidthData` | Input | **Binary Data** |
| `TextEncoding` | Input | Text |
| `RecordFormat` | Input | Text |
| `Success` | Output | Boolean |
| `RecordsJson` | Output | Text |
...
The `Binary Data` counterpart to `DecodeRecords` — for a caller whose bytes came from an SFTP `Get`, a file upload, or anywhere else that hands back `Binary Data` rather than `Text`. It exists so the byte↔char conversion happens inside PICASSO's own code: converting `Binary Data` to `Text` yourself first, the obvious thing to try, reaches for OutSystems' default UTF-8 handling and silently corrupts COMP-3/EBCDIC bytes — the same class of mistake as forgetting FTP binary mode. See [`docs/mainframe-ingestion.md`](../../docs/mainframe-ingestion.md) for the reasoning in full. `TextEncoding` and `RecordFormat` take the same names and same shorter overloads as `DecodeRecords`.

### `EncodeRecordsToBinary`
...
| `RecordFormat` | Input | Text |
| `Success` | Output | Boolean |
| `FixedWidthData` | Output | **Binary Data** |
| `ErrorMessage` | Output | Text |

The reverse: for handing bytes to something that wants `Binary Data`, like an SFTP `Put`.
...
public void MssDecodeRecordsFromBinary(string ssFlatSpecJson, byte[] ssFixedWidthData, string ssTextEncoding, string ssRecordFormat,
    out bool ssSuccess, out string ssRecordsJson, out string ssErrorMessage)
{
    ssSuccess = _picasso.DecodeRecordsFromBinary(ssFlatSpecJson, ssFixedWidthData, ssTextEncoding, ssRecordFormat,
        out ssRecordsJson, out ssErrorMessage);
}

public void MssEncodeRecordsToBinary(string ssFlatSpecJson, string ssRecordsJson, string ssTextEncoding, string ssRecordFormat,
    out bool ssSuccess, out byte[] ssFixedWidthData, out string ssErrorMessage)
{
    ssSuccess = _picasso.EncodeRecordsToBinary(ssFlatSpecJson, ssRecordsJson, ssTextEncoding, ssRecordFormat,
        out ssFixedWidthData, out ssErrorMessage);
}
```

`PicassoActions` holds no mutable state, so a single shared instance is fine. `Binary Data` maps to `byte[]` in the generated stub — same as `Text` maps to `string` — so those two methods need no special handling beyond matching the type Integration Studio actually generated.
...
1. In your app module: **Manage Dependencies** → find `Picasso` → tick the seven actions → **Apply**.
